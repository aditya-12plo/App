 ...
 <table class="table">
    ...
    <tbody v-if="this.orderanya.length > 0">
      <tr v-for="(row,index) in this.orderanya">
        ...
        <td align="left">{{row.header.drivername}} </td>
        <td align="left">
		<div v-for="rowyangkedua in row.detail">
...
Vue.component('my-button', require('./BooksCustomActions.vue'))
...
export default {
  ...
  data () {
	...
	address: '',
    errors: new Errors() ,
    forms:new CrudForm({index:'',  id:'',sku:'',name:'',description:'',price:'',order:''}) ,
   ...
   resetOptions() {
        ...
        showTime(kode,pesan) {
          this.$refs.toast.showToast(pesan, {
            ...
            orderItem(item ,index = this.indexOf(item)){
			this.forms.setFillItem(item , index );
                this.address='';
                this.modal.set('order', true);
            }  ,
        doFilter () {
...
onLoadingvuetableHistory() {
    ...
    onLoadingError() {
                this.loading = true;
                axios.get('/users/item').then((response) => {
    ...
    onLoadingErrorvuetableHistory() {
                this.loading = true;
                axios.get('/users/order-history').then((response) => {
...
var masuk ={
  ...
  orders:this.forms.orders,
 address:this.address};
 this.modal.set('order', false);
this.$swal({
  title: 'Are you sure ?',
  type: 'warning',
...
if (result.value) {
 ...
 created: function() {
  let self = this;
            this.$root.$on('orderitem', function(data,index){
                //console.log(data);
               self.orderItem(data,index);
            });
